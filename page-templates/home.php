...
get_header();
function get_news_from_category_field () {
...
function get_most_viewed_posts_ids () {
    $most_viewed_posts = kama_get_most_viewed("num=10 &echo=0 &return=array");
...
function get_popular_news (   ) {
    $news_from_category = get_news_from_category_field ();
    //ID выбранной в админкекатегории
    $news_from_category_id = get_news_from_category_id ($news_from_category);

    //IDs всех популярных постов
    ...
    if ( in_array($news_from_category_id, $most_viewed_posts_categories_ids) ) {
        $result = '';
        foreach ($most_viewed_posts_ids as $post_id) {
